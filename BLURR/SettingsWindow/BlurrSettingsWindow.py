...
class SettingsWindow():
    ...
    def __init__(self, objectTracker: ObjectTracker, setTracking, closeApp) -> None:
        self.isOpen = False
        self.objectTracker = objectTracker
        # initialize the services
        self.settings = SettingsService()
        self.recognition = RecognitionService(objectTracker)
        self.connectionService = ConnectionService(setTracking)
    ...
    def __runWindow__(self):
        ...
        try:
            eel.start('index.html', port=0, mode="chrome",
                      block=False, close_callback=onEnd)

...
class MainBlockerWindow():
    ...
    def show(self, message, password):
        ...
        bcg = ImageTk.PhotoImage(self.getBlurryBackground(0, 0, self.root.winfo_screenwidth(), self.root.winfo_screenheight()))
    ...
    def renderCenter(self):
        ...
        messageLbl = Label(text=self.msg, font=("Avenir Next", 18, 'bold'), justify=CENTER)
        messageLbl.grid(column=1, row=3, pady=5)

        blurrLabel = Label(text = "Blurr is activated", font=("Avenir Next", 18), justify=CENTER)
        blurrLabel.grid(column=1, row=4, pady=5)

    def renderRight(self):
        lbl = Label(text = "Press F4 to disable tracking and lock the station", font=("Avenir Next", 14), justify=CENTER,  cursor="hand2")
        lbl.grid(column=0, row=6, sticky=SW, pady=(0, 20), padx=(20,0))
        lbl = Label(text = "Developed with the Support of the Innovation Technology Campus – ITC in Munich", font=("Avenir Next", 8), justify=CENTER,  cursor="hand2")
        lbl.grid(column=2, row=6, sticky=SE, pady=(0, 20), padx=(0,20))


    def getBlurryBackground(self, x, y, width, height):
        sct = mss()
        img = numpy.array(sct.grab({"top": y, "left": x, "width": width, "height": height}))
        ...
        def do():
            ...
            for monitor in monitors:
                ...
                if not found:
                    ...
                    width =  round(monitor.width/ monitor.scaling * mainMonitor.scaling)
                    height = round(monitor.height/ monitor.scaling * mainMonitor.scaling)
                    ...
                    bcg = self.getBlurryBackground(monitor.x, monitor.y, width, height)
                    bcg.resize((1280, 720))
                    ...
                    print({'x': monitor.x, 'y': monitor.y, 'h': monitor.height, 'w': monitor.width, "overlay": overlay})
                    self.overlays.append({'x': monitor.x, 'y': monitor.y, 'h': monitor.height, 'w': monitor.width, "overlay": overlay})
